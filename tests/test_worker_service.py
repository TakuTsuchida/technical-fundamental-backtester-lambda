...
def _price_record(code: str, date: str = "2026-05-24") -> dict[str, Any]:
    return {
        "body": code,
        "messageAttributes": {
            "type": {"stringValue": "price", "dataType": "String"},
            "batch_date": {"stringValue": date, "dataType": "String"},
        },
    }


def _fins_record(code: str, date: str = "2026-05-24") -> dict[str, Any]:
    return {
        "body": code,
        "messageAttributes": {
...
class TestWorkerServiceProcessRecords:
    ...
    def test_calls_get_prices_for_each_code(self) -> None:
        ...
        WorkerService(deps).process_records(
            [_price_record("13010"), _price_record("72030"), _price_record("86970")]
        )
    ...
    def test_empty_records_returns_empty_list(self) -> None:
        ...
        mock_store = MagicMock()
        deps = WorkerDeps(price_fetcher=mock_jquants, fins_fetcher=MagicMock(), store=mock_store)
        saved = WorkerService(deps).process_records([])
        assert saved == []
        mock_store.put_json.assert_not_called()

    def test_key_uses_batch_date_from_record(self) -> None:
        mock_jquants = MagicMock()
        mock_jquants.get_prices_daily_quotes.return_value = []
        deps = WorkerDeps(price_fetcher=mock_jquants, fins_fetcher=MagicMock(), store=MagicMock())
    ...
    def test_no_type_attribute_defaults_to_price(self) -> None:
        mock_jquants = MagicMock()
        mock_jquants.get_prices_daily_quotes.return_value = []
        deps = WorkerDeps(price_fetcher=mock_jquants, fins_fetcher=MagicMock(), store=MagicMock())
        record = {
            "body": "13010",
            "messageAttributes": {
                "batch_date": {"stringValue": "2026-05-24", "dataType": "String"},
            },
        }
        saved = WorkerService(deps).process_records([record])
        assert saved == ["daily-prices/13010/2026-05-24.json"]
...
class TestWorkerServiceFinsRouting:
    def test_fins_record_returns_fins_summary_key(self) -> None:
        ...
        saved = WorkerService(deps).process_records([_fins_record("13010")])
    ...
    def test_fins_record_calls_get_fins_summary(self) -> None:
        ...
        mock_jquants.get_fins_summary.return_value = []
        deps = _make_deps(mock_jquants)
        WorkerService(deps).process_records([_fins_record("13010")])
    ...
    def test_mixed_records_routes_correctly(self) -> None:
        ...
        saved = WorkerService(deps).process_records(records)
        assert saved == [
...
class TestWorkerServiceBatchDate:
    def test_uses_batch_date_attribute(self) -> None:
        mock_jquants = MagicMock()
        mock_jquants.get_prices_daily_quotes.return_value = []
        deps = WorkerDeps(price_fetcher=mock_jquants, fins_fetcher=MagicMock(), store=MagicMock())
        record = {
    ...
    def test_raises_key_error_when_batch_date_absent(self) -> None:
        import pytest
        ...
        with pytest.raises(KeyError):
            WorkerService(deps).process_records([{"body": "13010"}])
